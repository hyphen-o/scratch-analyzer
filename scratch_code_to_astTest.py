...
class CodeToASTNode():
    ...
    def __init__(self, json_blocks):
        ...
        self.df_script = pd.DataFrame(
            [['SCRIPT', 0, -1]], columns=['block', 'node_id', 'parent_node_id'])  # 記録用のデータフレーム

    def get_result(self):
        return self.df_script

    def add_df(self, block, node_id, parent_node_id):
        se_add = pd.Series([block, node_id, parent_node_id],
                           index=self.df_script.columns)
    ...
    def depth_first_search(self, node_hash, parent_node_id):
        ...
        # 座標移動ブロックの場合に座標も出力する
        if block == 'motion_gotoxy':
            block += ' ' + \
                str(self.json_blocks[node_hash]['inputs']['X'][1][1]) + " " + \
                str(self.json_blocks[node_hash]['inputs']['Y'][1][1])
            self.add_df(block, self.node_id, parent_node_id)
        elif block == 'motion_movesteps':
            block += ' ' + \
                str(self.json_blocks[node_hash]['inputs']['STEPS'][1][1])
            self.add_df(block, self.node_id, parent_node_id)
        elif block == 'motion_glidesecstoxy':
            block += ' ' + \
                str(self.json_blocks[node_hash]['inputs']['X'][1][1]) + " " + \
                str(self.json_blocks[node_hash]['inputs']['Y'][1][1])
            self.add_df(block, self.node_id, parent_node_id)
        elif block == 'motion_changexby':
            block += ' ' + \
                str(self.json_blocks[node_hash]['inputs']['DX'][1][1])
            self.add_df(block, self.node_id, parent_node_id)
        elif block == 'motion_setx':
            block += ' ' + \
                str(self.json_blocks[node_hash]['inputs']['X'][1][1])
            self.add_df(block, self.node_id, parent_node_id)
        elif block == 'motion_changeyby':
            block += ' ' + \
                str(self.json_blocks[node_hash]['inputs']['DY'][1][1])
            self.add_df(block, self.node_id, parent_node_id)
        elif block == 'motion_sety':
            block += ' ' + \
                str(self.json_blocks[node_hash]['inputs']['Y'][1][1])
            self.add_df(block, self.node_id, parent_node_id)
        # 入力ブロックの場合に入力の種類も出力する
        elif block == 'sensing_keyoptions':
            block += ' ' + \
                str(self.json_blocks[node_hash]['fields']['KEY_OPTION'][0])
            self.add_df(block, self.node_id, parent_node_id)
        elif block == 'event_whenkeypressed':
            block += ' ' + \
                str(self.json_blocks[node_hash]['fields']['KEY_OPTION'][0])
            self.add_df(block, self.node_id, parent_node_id)
        else:
        ...
        if block in self.c_blocks:
            ...
            if block == 'control_if_else':
                ...
                if 'SUBSTACK' in self.json_blocks[node_hash]['inputs']:
                    self.depth_first_search(
                        self.json_blocks[node_hash]['inputs']['SUBSTACK'][1], self.node_id)

        if self.json_blocks[node_hash]['next'] == None:
            return
        else:
            self.depth_first_search(
                self.json_blocks[node_hash]['next'], parent_node_id)
...
# self.json_blocks[node_hash]['inputs']['DX'][1][1]
