          ...
          match category:
             ...
             case "VARIABLE":
                if block_name == constants.SET_VARIABLE:
                    self.__variables[block["fields"]["VARIABLE"][1]] = self.__get_variable_value(block["inputs"]["VALUE"])
                elif block_name == constants.CHANGE_VARIABLE:
                    self.__variables[block["fields"]["VARIABLE"][1]] = str(float(self.__variables[block["fields"]["VARIABLE"][1]]) + float(self.__get_variable_value(block["inputs"]["VALUE"])))
                self.__dfM.add_row([block_name, None, None, self.__node_id, self.__get_parent_index(block), block_hash])
                if block["next"]:     
                    self.__write_blocks(block['next'])
             case "CALL":
                ...
                for block_hash, block2 in self.__blocks.items():
                   if block2["opcode"] == constants.PROCEDURES_DEFINE:
                        if self.__blocks[block2["inputs"]["custom_block"][1]]["mutation"]["proccode"] == procedure_name:
                            if block2["next"]:
                                self.__dfM.add_row([block2["opcode"], None, None, self.__node_id, self.__get_parent_index(block2), block_hash])
                ...
                if block["inputs"] != None:
                   for key, input in block["inputs"].items():
                      if isinstance(block["inputs"][key][1], list):
                        block["inputs"][key][1][1] = self.__get_variable_value(input)
