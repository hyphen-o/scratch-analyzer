...
def toJson(project_id):
    ...
    try:
        project_token = scratch_client.get_token(project_id)
        json_data = scratch_client.get_project(project_id, project_token)
        return json_data
...
def writeBlocks(allblocks, block_hash, csv_name, field_name):
    try:
        ...
        with open(f'extracted_csv/{csv_name}', 'a') as f:
            writer = csv.writer(f)
            flg = False
            if (field_name):
...
def main(project_id):
    try:
        ...
        csv_name = f'{project_id}_extracted.csv'
        with open(f'extracted_csv/{csv_name}', 'w') as f:
            writer = csv.writer(f)
