...
def sameCoordinate(index):
    try:
        ...
        print(index)
        if (df_none.iloc[index + 1][1] == df_none.iloc[index - 1][1]) and (
            df_none.iloc[index + 1][2] == df_none.iloc[index - 1][2]
        ):
            if len(df_none.index) - 1 > index + 2:
                actions.key_down(KEY_DICT[str(df_none.iloc[index + 2][0])]).perform()
                time.sleep(0.1)
                actions.key_up(KEY_DICT[str(df_none.iloc[index + 2][0])]).perform()
...
def AutoInput():
    ...
    for i in range(1000000):
        try:
            ...
            print("not wait")
            if (
                len(df_none.index) - 1 > index
                and df_none.iloc[index + 1][0]
                and (
                    abs(round(df_none.iloc[index][1]) - int(X)) < 5
                    and abs(round(df_none.iloc[index][2]) - int(Y)) < 5
                )
            ):
                ...
                index = index + 1
                print(index)
                actions.key_down(KEY_DICT[str(df_none.iloc[index][0])]).perform()
                time.sleep(0.1)
                actions.key_up(KEY_DICT[str(df_none.iloc[index][0])]).perform()
