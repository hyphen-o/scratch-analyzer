...
class CodeToASTNode:
    ...
    def depth_first_search(self, node_hash, parent_node_id):
        ...
        if block == "motion_gotoxy":
            ...
            block += " " + str(self.json_blocks[node_hash]["inputs"]["STEPS"][1][1])
            self.add_df(block, self.node_id, parent_node_id)
            ...
            block += " " + str(self.json_blocks[node_hash]["inputs"]["DX"][1][1])
            self.add_df(block, self.node_id, parent_node_id)
        elif block == "motion_setx":
            block += " " + str(self.json_blocks[node_hash]["inputs"]["X"][1][1])
            self.add_df(block, self.node_id, parent_node_id)
        elif block == "motion_changeyby":
            block += " " + str(self.json_blocks[node_hash]["inputs"]["DY"][1][1])
            self.add_df(block, self.node_id, parent_node_id)
        elif block == "motion_sety":
            block += " " + str(self.json_blocks[node_hash]["inputs"]["Y"][1][1])
            self.add_df(block, self.node_id, parent_node_id)
        # 入力ブロックの場合に入力の種類も出力する
        elif block == "sensing_keyoptions":
            block += " " + str(self.json_blocks[node_hash]["fields"]["KEY_OPTION"][0])
            self.add_df(block, self.node_id, parent_node_id)
